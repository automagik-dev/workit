...
- Secrets: respect empty `WK_KEYRING_PASSWORD` (treat set-to-empty as intentional; avoids headless prompts). (#269) — thanks @zerone0x.
...
- Secrets: set keyring item labels to `workit` so macOS security prompts show a clear item name. (#106) — thanks @maxceem.
...
- Drive: `wk drive drives` lists shared drives (Team Drives). (#67) — thanks @pasogott.
- Sheets: `wk sheets format` applies cell formatting via `--format-json` + `--format-fields`. (#72) — thanks @nilzzzzzz.

### Changed

- Tasks: `wk tasks list` now defaults to `--show-assigned`. (#59) — thanks @tompson.

## 0.6.0 - 2026-01-11

### Added

- Auth: Workspace service accounts (domain-wide delegation) for all services via `wk auth service-account ...` (preferred when configured). (#54) — thanks @pvieito.

### Fixed

- Keep: use `keep.readonly` scope (service account). (#64) — thanks @jeremys.
- Sheets: `wk auth add --services sheets --readonly` now includes Drive read-only scope so `wk sheets export` works. (#62)
...
- Auth: `wk auth add --readonly` and `--drive-scope` for least-privilege tokens. (#58) — thanks @jeremys.

### Fixed

- Paths: expand leading `~` in user-provided file paths (e.g. `--out "~/Downloads/file.pdf"`). (#56) — thanks @salmonumbrella.
- Calendar: accept ISO 8601 timezones without colon (e.g. `-0800`) and add `wk calendar list` alias. (#56) — thanks @salmonumbrella.

## 0.5.3 - 2026-01-10

### Fixed

- CLI: infer account when `--account`/`WK_ACCOUNT` not set (uses keyring default or single stored token).

## 0.5.2 - 2026-01-10

### Fixed

- Release builds: embed version/commit/date so `wk --version` is correct (Homebrew/tap installs too).
...
- Email open tracking: `wk gmail send --track` + `wk gmail track ...` (Cloudflare Worker backend; optional per-account setup + `--track-split`) (#35) — thanks @salmonumbrella.
...
- Auth + config: JSON5 `config.json`, improved `wk auth status`, `wk auth keyring ...`, and refresh token validation via `wk auth list --check`.
...
- Calendar:
  - `wk calendar create|update --rrule/--reminder` for recurrence rules and custom reminders — thanks @salmonumbrella.
  - `wk calendar update --add-attendee ...` to add attendees without losing existing RSVP state.
  - Workspace users list + timezone-aware time windows and flags like `--week-start`.
- Gmail:
  - `wk gmail thread attachments` list/download attachments (#27) — thanks @salmonumbrella.
  - `wk gmail thread get --full` shows complete bodies (default truncates) (#25) — thanks @salmonumbrella.
  - `wk gmail labels create`, reply-all support, thread search date display, and thread-id replies.
  - `wk gmail get --json` includes flattened headers, `unsubscribe`, and extracted `body` (for `--format full`).
  - `wk gmail settings ...` reorg + filter operations now request the right settings scope (thanks @camerondare).
- Keep: list/search/get notes and download attachments (Workspace only; service account via `wk auth keep ...`) — thanks @koala73.
- Contacts: `wk contacts other delete` for removing other contacts (thanks @salmonumbrella).
...
- Auth/services:
  ...
  - `groups` service support for Cloud Identity (Workspace only): `wk auth add <email> --services groups`.
  - `wk auth keyring <auto|keychain|file>` writes `keyring_backend` to `config.json`.
  - `WK_KEYRING_BACKEND={auto|keychain|file}` to force a backend (use `file` to avoid Keychain prompts; pair with `WK_KEYRING_PASSWORD`).
...
- Auth: `wk auth tokens list` filters non-token keyring entries.
...
- CLI: help output polish (grouped by default, optional full expansion via `WK_HELP=full`); colored headings/command names; more flag aliases like `--output`/`--output-dir` (#47) — thanks @salmonumbrella.
- Homebrew/DX: tap installs GitHub release binaries (macOS) to reduce Keychain prompt churn; remove pnpm wrapper in favor of `make wk` targets; `make wk <args>` works without `ARGS=`.
- Auth: `wk auth add` now defaults to `--services user` (`--services all` remains accepted for backwards compatibility).
...
- Gmail: `wk gmail thread --download --out-dir ...` for saving thread attachments to a specific directory.
- Calendar: colors, conflicts, search, multi-timezone time.
- Sheets: read/write/update/append/clear + create spreadsheets.
- Sheets: copy spreadsheets via Drive (`wk sheets copy ...`).
- Drive: `wk drive download --format ...` for Google Docs exports (e.g. Sheets to PDF/XLSX, Docs to PDF/DOCX/TXT, Slides to PDF/PPTX).
- Drive: copy files (`wk drive copy ...`).
- Docs/Slides/Sheets: dedicated export commands (`wk docs export`, `wk slides export`, `wk sheets export`).
- Docs: create/copy (`wk docs create`, `wk docs copy`) and print plain text (`wk docs cat`).
- Slides: create/copy (`wk slides create`, `wk slides copy`).
- Auth: browser-based accounts manager (`wk auth manage`).
- DX: shell completion (`wk completion ...`) and `--verbose` logging.

### Fixed

- Gmail: `wk gmail attachment` download now works reliably; avoid re-fetching payload for filename inference and accept padded base64 responses.
- Gmail: `wk gmail thread --download` now saves attachments to the current directory by default and creates missing output directories.
...
- Calendar: `wk calendar calendars` and `wk calendar acl` now support `--max` and `--page` (JSON includes `nextPageToken`).
- Drive: `wk drive permissions` now supports `--max` and `--page` (JSON includes `nextPageToken`).

### Changed

- macOS: stop trying to modify Keychain ACLs (“trust wk”); removed `WK_KEYCHAIN_TRUST_APPLICATION`.
- BREAKING: remove positional/legacy flags; normalize paging and file output flags.
- BREAKING: replace `--output` with `--json` and `--plain` (and env `WK_OUTPUT` with `WK_JSON`/`WK_PLAIN`).
- BREAKING: destructive commands now require `--force` in non-interactive contexts (or they prompt on TTY).
- BREAKING: `wk calendar create|update` uses `--from/--to` (removed `--start/--end`).
- BREAKING: `wk gmail send|drafts create` uses `--reply-to-message-id` (removed `--reply-to` for message IDs) and `--reply-to` (removed `--reply-to-address`).
- BREAKING: `wk gmail attachment` uses `--name` (removed `--filename`).
...
- BREAKING: `wk drive download` uses `--out` (removed positional `destPath`).
- BREAKING: `wk auth tokens export` uses `--out` (removed positional `outPath`).
- BREAKING: `wk auth tokens export` uses `--overwrite` (removed `--force`).

## 0.2.1 - 2025-12-26

### Fixed

- macOS: reduce repeated Keychain password prompts by trusting the `wk` binary by default (set `WK_KEYCHAIN_TRUST_APPLICATION=0` to disable).

## 0.2.0 - 2025-12-24

### Added

- Gmail: watch + Pub/Sub push handler (`wk gmail watch start|status|renew|stop|serve`) with optional webhook forwarding, include-body, and max-bytes.
- Gmail: history listing via `wk gmail history --since <historyId>`.
...
- Build: `make` builds `./bin/wk` by default (adds `build` target, PR #12 — thanks @advait).
- Docs: local build instructions now use `make` (PR #12 — thanks @advait).

### Fixed

- Secrets: keyring file-backend fallback now stores encrypted entries in `$(os.UserConfigDir())/workit/keyring/` and supports non-interactive via `WK_KEYRING_PASSWORD` (PR #13 — thanks @advait).
- Gmail: decode base64url attachment/message-part payloads (PR #15 — thanks @shanelindsay).
- Auth: add `people` service (OIDC `profile` scope) so `wk people me` works with `wk auth add --services all`.

## 0.1.1 - 2025-12-17

### Added

- Calendar: respond to invites via `wk calendar respond <calendarId> <eventId> --status accepted|declined|tentative` (optional `--send-updates`).
- People: `wk people me` (quick “me card” / `people/me`).
- Gmail: message get via `wk gmail get <messageId> [--format full|metadata|raw]`.
- Gmail: download a single attachment via `wk gmail attachment <messageId> <attachmentId> [--out PATH]`.

## 0.1.0 - 2025-12-12

Initial public release of `wk`: a single Go CLI that unifies Gmail, Calendar, Drive, and Contacts (People API).

### Added

- Unified CLI (`wk`) with service subcommands: `gmail`, `calendar`, `drive`, `contacts`, plus `auth`.
- OAuth setup and account management:
  - Store OAuth client credentials: `wk auth credentials <credentials.json>`.
  - Authorize accounts and store refresh tokens securely via OS keychain using `github.com/99designs/keyring`.
  - List/remove accounts: `wk auth list`, `wk auth remove <email>`.
  - Token management helpers: `wk auth tokens list|delete|export|import`.
...
- Developer experience:
  ...
  - `make` builds `./bin/wk` for local dev (`make && ./bin/wk auth add you@gmail.com`).
