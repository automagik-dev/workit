...
`wk contacts update` supports JSON input via `--from-file`, so you can update People API fields without adding new CLI flags.
...
```bash
wk contacts get people/c123456 --json > contact.json

# Edit contact.json (see notes below)
wk contacts update people/c123456 --from-file contact.json
```

Update from stdin:

```bash
wk contacts get people/c123456 --json | \
  jq '(.contact.urls //= []) | (.contact.urls += [{"value":"https://example.com","type":"profile"}])' | \
  wk contacts update people/c123456 --from-file -
```
...
- Wrapped (from `wk contacts get --json`): `{"contact": { ...person... }}`
...
If the JSON contains unsupported fields (for `updateContact`), wk errors instead of silently ignoring them.
...
To avoid overwriting concurrent contact edits, wk compares the JSON etag with the current contact etag:
