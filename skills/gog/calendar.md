...
Google Calendar (`gog calendar` / alias `cal`).

Use `--read-only` for inspection, `--dry-run` for writes, and `--json`/`--plain` for automation.

## Top-level commands (from `gog calendar --help`)
- `calendars`
- `acl <calendarId>`
- `events [calendarId]`
...
## Examples
```bash
gog calendar events primary --from 2026-02-18T09:00:00-03:00 --to 2026-02-18T18:00:00-03:00 --read-only --plain
