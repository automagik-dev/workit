...
func TestWrapOAuthError_UnauthorizedClient(t *testing.T) {
	orig := errors.New("oauth2: unauthorized_client: something went wrong")
...
func TestWrapOAuthError_InvalidGrant(t *testing.T) {
	orig := errors.New("oauth2: invalid_grant: Token has been revoked")
	wrapped := WrapOAuthError(orig)
	if wrapped == nil {
		t.Fatal("expected non-nil error")
	}
...
func TestWrapOAuthError_InvalidClient(t *testing.T) {
	orig := errors.New("oauth2: invalid_client: The OAuth client was not found")
	wrapped := WrapOAuthError(orig)
	if wrapped == nil {
		t.Fatal("expected non-nil error")
	}
...
func TestWrapOAuthError_UnknownPassthrough(t *testing.T) {
	orig := errors.New("some totally unrelated error")
	wrapped := WrapOAuthError(orig)
	if wrapped != orig {
		t.Fatalf("unknown error should pass through unchanged, got: %v", wrapped)
	}
}

func TestWrapOAuthError_WrappedOriginalPreserved(t *testing.T) {
	// Verify errors.Is works through the wrapping chain
	inner := errors.New("invalid_grant: bad token")
