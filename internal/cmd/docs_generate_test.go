...
func TestDocsGenerate_JSON_NonStringValues(t *testing.T) {
	origDocs := newDocsService
	origDrive := newDriveService
	t.Cleanup(func() {
		newDocsService = origDocs
	...
	for _, reqRaw := range requests {
		...
		if !ok {
			t.Fatalf("request item should be object, got: %T", reqRaw)
		}
		replaceAllText, ok := reqMap["replaceAllText"].(map[string]any)
		if !ok {
			t.Fatalf("request missing replaceAllText: %v", reqMap)
		}
		containsText, ok := replaceAllText["containsText"].(map[string]any)
		if !ok {
			t.Fatalf("replaceAllText missing containsText: %v", replaceAllText)
		}
		placeholder, _ := containsText["text"].(string)
		replaceText, _ := replaceAllText["replaceText"].(string)
		got[placeholder] = replaceText
	}
	...
	for key, value := range want {
		if got[key] != value {
			t.Fatalf("placeholder %s => %q, want %q", key, got[key], value)
		}
	}
}
...
func TestDocsGenerate_InvalidDataJSON(t *testing.T) {
	...
	errUnexpectedCall := errors.New("unexpected service call")
