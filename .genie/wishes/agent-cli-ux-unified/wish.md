...
### IN
- `gog help <topic>` with hybrid schema-generated + curated concept docs
- Field discovery via `--select` (empty value) lists available JSON fields
...
- AGENTS.md updated with all new discovery features
...
- [ ] Agent discovers auth setup via `gog help auth`, runs `gog login`, verifies with `gog status`
- [ ] Agent discovers selectable fields via `--select` (empty), lists Drive with projected fields
- [ ] Agent discovers input flags via `--generate-input`, constructs and sends an email

### Secondary: Technical Criteria
- [ ] `gog help topics` lists all topics; `gog help agent` renders hybrid content
- [ ] `gog drive ls --json --select` (empty) prints fields to stderr, exits 0
...
- **ASM-3:** Existing per-command `--max`/`--limit`/`--page` flags can be aliased to new global pagination flags without breaking existing behavior.

## Risks

- **RISK-1:** Help topic concept prose drifts from code — Mitigation: keep curated sections minimal; lean on schema-generated facts; CI check that topic references valid commands.
...
- **RISK-3:** Pagination flags conflict with existing per-command `--max`/`--page` — Mitigation: audit all commands; make `--max` alias for `--max-results`; keep `--page` if semantically different.
...
**Goal:** Add `gog help <topic>` providing concept-level docs with live schema facts that never drift.
...
- Modify: `internal/cmd/root.go` — register `help` command at root level

**Acceptance Criteria:**
- [ ] `gog help topics` lists all available topics with descriptions
- [ ] `gog help agent` renders agent integration guide with live schema facts (services, exit codes)
- [ ] `gog help auth` explains authentication workflow
- [ ] `gog help output` explains --json, --plain, --select, --results-only
...
**Goal:** `gog drive ls --json --select` (empty value) lists available JSON fields without making an API call.
...
**Acceptance Criteria:**
- [ ] `gog drive ls --json --select` (no value) prints available field names to stderr
...
- Modify: `internal/cmd/paging.go` — wire global flags into `collectAllPages` and per-command fetch functions
...
- [ ] `--results-only` still strips envelope fields
...
internal/cmd/root.go                    # Modify — register help command
...
internal/cmd/paging.go                  # Modify — wire flags
